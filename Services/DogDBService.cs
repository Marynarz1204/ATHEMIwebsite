...
public class DogDBService
{
    ...
    public DogDBService(IOptions<MongoDBSettings> mongoDBSettings)
    {
    ...
    public async Task<Dog> GetDogByIDAsync(string id)
    {
        FilterDefinition<Dog> filter = Builders<Dog>.Filter.Eq("DogId", id);
        return await _dogCollection.Find(filter).Limit(1).SingleAsync();
    }
